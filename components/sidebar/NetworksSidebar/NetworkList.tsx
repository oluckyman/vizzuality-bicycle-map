...
export function NetworkList({ networks, countryNames }: NetworkListProps) {
  return (
          ...
          <li key={network.id} className="group border-b px-6 py-4  hover:bg-accent transition-colors duration-250">
            <Link href={`/network/${network.id}`} className="flex flex-col gap-1">
              ...
              <div className="flex items-end justify-between">
                ...
                <Button variant={"secondary"} className="text-grenadier-500 w-26 gap-1 cursor-pointer justify-end">
                  <span className="overflow-hidden max-w-0 group-hover:max-w-xs transition-all duration-250">
                    Details
                  </span>
